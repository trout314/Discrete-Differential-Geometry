...
def collect_samples(manifold, params, *, num_samples, burn_in_moves, spacing_moves, exact):
    ...
    for i in range(num_samples):
        ...
        fv = sampler.f_vector
        num_facets.append(fv[len(fv) - 1])  # top dimension
    ...
    return {
        "num_facets": np.array(num_facets, dtype=float),
...
def main():
    ...
    manifold_dim = int(sys.argv[4]) if len(sys.argv) > 4 else 3

    seed = Manifold.standard_sphere(manifold_dim)
    ...
    print(f"Params: dim={manifold_dim}, target={target}, facet_coef={facet_coef}, spacing={spacing}")
    ...
    print(f"\nResults (n={n}, target={target} tets, KS critical={ks_critical:.4f}):")
    all_pass = True
    for key in ["num_facets", "num_edges"]:
        ...
        if weighted_ok and unweighted_differs:
            ...
            status = "WEAK PASS (weights correct but unweighted also passes — test is not discriminating)"
        elif not weighted_ok:

...
- [Contract/split DB verdict](contract-split-db-verdict.md) — channel EXONERATED (exactly reversible by enumeration, <=1% by rate audit); circulation CANNOT localise (an f0-conserving biased kernel makes both f0 families circulate); + run(exact=True) is not exact
...
- [Quanta strain & heal](quanta-strain-heal.md) — 1 quantum = 1 tet = 1 sixfold edge; a15's two strain directions are KINETICALLY INEQUIVALENT: above-native heals to 100% pristine registry, below-native is unreachable from pristine by any PRICE (melt or refuse) but reachable with a BUDGET and stable once there; exact 4-species flicker spectrum

...
THETA = mg.THETA_TET   # still used for the ledger deficit-velocity (K2d), not the static field
ENSEMBLES = [
...
def fields_from_facets(F):
    """Per-vertex degree D_v and Regge deficit sum Sum_{e ni v} delta_e, as dicts
    keyed by RAW sampler label (to combine with the raw-label geometry ledger).
    The deficit sum is 2 * the shared curvature charge q_R = 1/2 Sum delta, which
    is returned in np.unique(F) = lab order -- so lab[i] <-> the field's index i."""
    lab, cnt = np.unique(F, return_counts=True)
    D = dict(zip(lab.tolist(), cnt.tolist()))
    qR = FIELDS["curvature_charge"](F)          # = dsum/2, in lab order
    dsum = {int(v): float(2 * qR[i]) for i, v in enumerate(lab)}
    return D, dsum

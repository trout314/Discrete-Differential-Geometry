...
Per class this reports:
  ...
  SHIFT s -- a GLOBAL coincidence of the closed loop, NOT a period of the helix
      An automorphism commutes with the sliding-window map, so any g fixing
      the chain setwise sends window k to window k+s for a fixed shift s; the
      realized shifts form a subgroup of Z_L generated by s, with L/s =
      |Stab| elements.

      Read this carefully, because it is a standing source of confusion. The
      BC helix has NO translational symmetry along its axis -- its step twist
      is arccos(-2/3), an irrational multiple of pi, which is precisely what
      the HOLONOMY entry below measures (R^L != I for every L, so the helix
      never repeats). Nor is there period-4 structure, despite the sliding
      window being four vertices wide: measured on R m4, NO shift of 1..12 --
      4 included -- is a symmetry, and the smallest one that is, is
      271 = L/12. s is a fact about how this particular chain happens to CLOSE
      on the torus, found by searching Aut and never assumed. And since Aut is
      combinatorial and metric-blind (see symmetry.py), s need not correspond
      to any Euclidean screw operation at all.

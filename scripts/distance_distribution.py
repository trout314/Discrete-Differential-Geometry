...
Distances come from unweighted BFS. Computing *all* pairs is O(V*E); instead we
sample a set of source nodes uniformly and histogram the distances from each to
every other node. By homogeneity the single-source distance distribution equals
the pairwise one, so sampling sources is unbiased (and we average over the
provided seeds/replicas to reduce noise). Pass ``--sources all`` for exact
all-pairs on small graphs.
...
def summarize(hist):
    """Mean, median, and max (diameter proxy) from a shell histogram."""
    r = np.arange(len(hist))
    total = hist.sum()
    if total == 0:
        return dict(mean=0.0, median=0, diameter=0, pairs=0)
    mean = (r * hist).sum() / total
    cdf = np.cumsum(hist) / total
    median = int(np.searchsorted(cdf, 0.5))
    diameter = int(r[hist > 0][-1])
    return dict(mean=float(mean), median=median, diameter=diameter, pairs=int(total))


def analyze(seed_paths, dim, n_sources, rng):
    """Aggregate edge- and dual-graph distance histograms over the given seeds."""
    agg = {"edge": np.zeros(1, dtype=np.int64), "dual": np.zeros(1, dtype=np.int64)}
    n_nodes = {"edge": 0, "dual": 0}
    n_srcs = {"edge": 0, "dual": 0}
    with tempfile.TemporaryDirectory() as tmp:
        for path in seed_paths:
            ...
            for kind, (saver, n_hint) in savers.items():
    ...
    with open(path, "w") as f:
        ...
        for kind, hist in agg.items():
            ...
            for r, c in enumerate(hist):
                cum += c
                f.write(f"{kind},{r},{int(c)},{c/total:.8g},{cum/total:.8g}\n")


def maybe_plot(path, agg):
    try:
        import matplotlib
        matplotlib.use("Agg")
    ...
    for kind, hist in agg.items():
        total = hist.sum() or 1
        r = np.arange(len(hist))
        ax.plot(r, hist / total, marker=".", ms=4, lw=1, label=f"{kind}-distance")
    ax.set_xlabel("distance r"); ax.set_ylabel("P(d = r)")
    ax.set_title("Graph-distance distribution"); ax.legend()
...
def main():
    ...
    p.add_argument("--sources", default="1000",
                   help="Number of BFS source nodes, or 'all' for exact all-pairs "
                        "(default 1000).")
    ...
    p.add_argument("--plot", default=None, help="Write a PNG overlay of the two PDFs.")
    args = p.parse_args()
    ...
    print(f"Analyzing {len(paths)} seed(s), dim={args.dim}, "
          f"sources={'all' if n_sources is None else n_sources}", flush=True)
    agg, n_nodes, n_srcs = analyze(paths, args.dim, n_sources, rng)

    print(f"\n{'graph':>6} {'nodes':>9} {'sources':>8} {'pairs':>12} "
          f"{'mean':>8} {'median':>7} {'diameter':>9}")
    for kind in ("edge", "dual"):
        s = summarize(agg[kind])
        print(f"{kind:>6} {n_nodes[kind]:>9} {n_srcs[kind]:>8} {s['pairs']:>12} "
              f"{s['mean']:>8.3f} {s['median']:>7} {s['diameter']:>9}")

    if args.csv:
        write_csv(args.csv, agg); print(f"\nwrote {args.csv}")
    if args.plot:
        maybe_plot(args.plot, agg)

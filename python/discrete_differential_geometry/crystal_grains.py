...
Promoted from scripts/crystal_grains.py (2026-08 cleanup, Phase 1a); that
file is now a shim whose CLI dispatches to `main` here.
...
from .symmetry import CrystalSymmetry, TriView, develop_partial
from .fk_skeleton import edges_from_facets, vertex_classes
from .tcp_reference import STRUCTURES

# Repo root: REF_GLOB assumes the source-tree layout (the reference crystals
# are gitignored data; see the regenerate-tcp-references notes).
...
def main():
    ...
    refs = {n: build_struct(np.asarray(Manifold.load(p, 3).facets()))
    ...
    results = {}
    for f in files:
        facets = np.asarray(Manifold.load(f, 3).facets())

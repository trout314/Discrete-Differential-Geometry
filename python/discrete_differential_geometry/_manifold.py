...
class Manifold:
    ...
    def count_valid_hinge_moves(self) -> int:
        """Number of valid 4-4 hinge moves (dim = 3 only).

        One per degree-4 edge per link-cycle diagonal whose edge is not
        already present, so 0, 1 or 2 per degree-4 edge. They carry c = 2
        apiece in the proposal normaliser -- see :meth:`importance_weight`.
        """
        return _lib.ddg_manifold_count_valid_hinge_moves(self._handle)

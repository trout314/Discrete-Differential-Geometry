"""T^3 triangulations of real tetrahedrally-close-packed (TCP) crystals.
...
them. `STRUCTURES` carries published Wyckoff positions of the classical TCP
phases; `build_t3_triangulation` builds an m x m x m supercell, computes the
periodic Delaunay triangulation, and quotients it to a simplicial
triangulation of the 3-torus.
...
Method notes:
  ...
  * m >= 2 required (m >= 3 recommended) for a simplicial quotient.

Promoted from scripts/tcp_reference.py (2026-08 cleanup, Phase 1a); the
validate-and-save CLI stays there.
"""
import numpy as np
...
def reference_frac_positions(name, m, perturb=1e-6):
    ...
    (Promoted from scripts/cocycle_check.py, which re-exports it for its many
    legacy importers.)
    """
    L, sites, cn, _ = STRUCTURES[name]
    ns = len(sites)

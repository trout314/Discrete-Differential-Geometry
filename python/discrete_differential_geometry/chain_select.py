...
Promoted from tools/chain_select.py (2026-08 cleanup, Phase 1a); that file is
now a shim.
"""
...
def reference_positions(struct, m, nvert):
    ...
    Thin validation wrapper over ``tcp_reference.reference_frac_positions``
    (site perturbation and ``v = cell*ns + site`` id scheme live there).
    """
    from .tcp_reference import STRUCTURES, reference_frac_positions
    if struct not in STRUCTURES:
        return None
    ns = len(STRUCTURES[struct][1])
    if ns * m ** 3 != nvert:
        return None
    return reference_frac_positions(struct, m)
